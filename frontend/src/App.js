...
function App() {
  ...
  const [linkedAccounts, setLinkedAccounts] = useState([
    {
      id: "1",
      name: "Nandini",
      email: "nandueduka@gmail.com",
      status: "active",
    },
    {
      id: "2",
      name: "NANDINI N.",
      email: "1ms22is085@msrit.edu",
      status: "signed out",
    },
  ]);
    ...
    async (currentUserId) => {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/chat/history/${currentUserId}`,
        );
        if (!response.ok)
  ...
  const handleClearHistory = async () => {
    ...
    if (
      window.confirm(
        `Are you sure you want to permanently delete all chat history for user '${username}'? This action cannot be undone.`,
      )
    ) {
      try {
        const response = await fetch(
          `${API_BASE_URL}/chat/history/${username}`,
          {
            method: "DELETE",
          },
        );
      ...
      return (
        <div className="flex flex-col items-center justify-center h-full p-6 bg-gray-100">
          <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md border">
            ...
            {!isSigningUp && (
              <p className="text-center text-sm mt-2">Hint: user / password</p>
            )}
          </div>
        </div>
      );
              ...
              {sessionId ? (
                ...
                <>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="https://www.icicibank.com/personal-banking/loans/personal-loan/service-charges"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold mb-1 text-gray-700">
                      ...
                      <input
                        ...
                        className="w-full p-2 border border-gray-300 rounded-lg"
                        placeholder="https://www.hdfcbank.com/personal/borrow/popular-loans/personal-loan/fees-and-charges"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold mb-1 text-gray-700">
                      ...
                      <textarea
                        ...
                        placeholder={
                          language === "en"
                            ? "e.g., Compare these two based on their pre-payment penalties..."
                            : "ಉದಾಹರಣೆಗೆ, ಇವುಗಳ ಪೂರ್ವ-ಪಾವತಿ ದಂಡಗಳನ್ನು ಹೋಲಿಕೆ ಮಾಡಿ..."
                        }
                        rows="2"
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      {samplePrompts[language].map((prompt, index) => (
                        <button
                          ...
                          className="text-xs text-left bg-gray-200 hover:bg-gray-300 p-2 rounded-lg transition-colors"
                        >
                          {prompt}
                        </button>
                      ))}
                  ...
                  <button
                    ...
                    className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-lg disabled:bg-gray-400"
                  >
                    {isProcessing
                      ? language === "en"
                        ? "Processing..."
                      ...
                      : language === "en"
                        ...
                        : "URL ಗಳನ್ನು ಪ್ರಕ್ರಿಯೆಗೊಳಿಸಿ ಮತ್ತು ವಿಶ್ಲೇಷಿಸಿ"}
                  </button>
                </>
              )}
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-200">
              {ragConversation.length === 0 && !isProcessing && (
                <div className="text-center text-gray-500 mt-10 p-4">
                  ...
                  <p className="text-lg">
                    {language === "en"
                      ? "Enter two URLs and ask a question to begin."
                      : "ಎರಡು URL ಗಳನ್ನು ನಮೂದಿಸಿ ಮತ್ತು ಪ್ರಾರಂಭಿಸಲು ಒಂದು ಪ್ರಶ್ನೆಯನ್ನು ಕೇಳಿ."}
                  </p>
                </div>
              )}
              {ragConversation.map((msg, index) => (
                ...
                >
                  <div
                    className={`max-w-3xl p-4 rounded-xl shadow-md ${msg.sender === "user" ? "bg-blue-600 text-white rounded-br-none" : "bg-white text-gray-800 rounded-bl-none border"}`}
                  >
                    <div
        ...
        return (
          <div className="flex flex-col items-center justify-start h-full p-6 text-gray-700 bg-white rounded-lg shadow-inner overflow-y-auto">
            <h2 className="text-3xl font-bold text-gray-800 mb-6 mt-4">
              {language === "en" ? "Your Chat History" : "ನಿಮ್ಮ ಚಾಟ್ ಇತಿಹಾಸ"}
            </h2>
            <div className="w-full max-w-3xl bg-gray-50 p-6 rounded-xl shadow-md border border-gray-200 mb-4">
              <div className="flex justify-end items-center mb-4">
                ...
                >
                  {language === "en" ? "Clear History" : "ಇತಿಹಾಸ ಅಳಿಸಿ"}
                </button>
              </div>
              {historicalMessages.length === 0 ? (
                ...
                <div className="overflow-y-auto max-h-[calc(100vh-300px)] custom-scrollbar">
                  {historicalMessages.map((msg) => (
                    <div
                      key={msg.id}
                      className={`mb-4 last:mb-0 ${msg.sender === "user" ? "text-right" : "text-left"}`}
                    >
                      <div
                        className={`inline-block p-3 rounded-lg max-w-[80%] ${msg.sender === "user" ? "bg-blue-100 text-blue-800" : "bg-gray-100 text-gray-800"}`}
                      >
          ...
          <div className="flex flex-col items-center justify-start h-full p-6 text-gray-700 bg-gray-100 rounded-lg shadow-inner overflow-y-auto">
            <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md border border-gray-200">
              <div className="flex items-center space-x-4 pb-6 border-b border-gray-200 mb-6">
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    {email || `${username}@finease.example.com`}
                  </p>
                  <button className="mt-2 text-blue-600 hover:underline text-sm">
                    {language === "en"
                      ? "Manage your Finease Account"
                      : "ನಿಮ್ಮ ಫಿನೀಸ್ ಖಾತೆಯನ್ನು ನಿರ್ವಹಿಸಿ"}
                  </button>
                </div>
              </div>
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">
                  {language === "en" ? "Other Accounts" : "ಇತರ ಖಾತೆಗಳು"}
                </h3>
                      ...
                      <div className="flex space-x-2">
                        {account.status === "signed out" ? (
                          <button
                            ...
                            onClick={() =>
                              handleLinkedAccountSignIn(account.id)
                            }
                          >
              ...
              <div className="space-y-3 pt-6 border-t border-gray-200">
                <button className="w-full flex items-center justify-center bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-3 px-4 rounded-lg">
                  <i className="fas fa-plus mr-2"></i>{" "}
                  {language === "en"
                    ? "Add another account"
                    : "ಇನ್ನೊಂದು ಖಾತೆ ಸೇರಿಸಿ"}
                </button>
                <button
                  className="w-full flex items-center justify-center bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-3 px-4 rounded-lg"
                  onClick={handleSignOut}
                >
                  <i className="fas fa-sign-out-alt mr-2"></i>{" "}
                  {language === "en"
                    ? "Sign out of all accounts"
                    : "ಎಲ್ಲಾ ಖಾತೆಗಳಿಂದ ಸೈನ್ ಔಟ್ ಮಾಡಿ"}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col h-full bg-gray-50">
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {faqMessages.length === 0 ? (
                <div className="text-center text-gray-500 mt-10">
                  <div className="mb-8">
                    {language === "en"
                      ? "Welcome! Ask me a general financial question, or try one of these:"
                      : "ಸ್ವಾಗತ! ಸಾಮಾನ್ಯ ಹಣಕಾಸಿನ ಪ್ರಶ್ನೆಯನ್ನು ಕೇಳಿ, ಅಥವಾ ಇವುಗಳಲ್ಲಿ ಒಂದನ್ನು ಪ್ರಯತ್ನಿಸಿ:"}
                  </div>
                ...
                faqMessages.map((msg) => (
                  ...
                  >
                    <div
                      className={`max-w-2xl p-4 rounded-xl shadow-md ${msg.sender === "user" ? "bg-blue-600 text-white rounded-br-none" : "bg-gray-200 text-gray-800 rounded-bl-none"}`}
                    >
                      {msg.message}
                    </div>
                  </div>
            ...
            >
              <div className="flex items-center space-x-4 max-w-3xl mx-auto">
                <input
                  ...
                  placeholder={
                    language === "en"
                      ...
                      : "ಯಾವುದೇ ಹಣಕಾಸು ಸಂಬಂಧಿತ ಪ್ರಶ್ನೆಗಳನ್ನು ಕೇಳಿ..."
                  }
                  disabled={isFaqSending}
                />
                ...
                <button
                  type="submit"
                  className="bg-blue-600 hover:bg-blue-700 text-white p-3 rounded-lg disabled:bg-gray-400"
                  disabled={isFaqSending || !faqInput.trim()}
                >
                  Send
                </button>
              </div>
    ...
    <div className="flex h-screen bg-gray-900 font-inter">
      ...
      <div className="w-64 bg-gray-900 text-white flex flex-col shadow-2xl">
        <div className="p-6 flex justify-center items-center border-b border-gray-700">
          <img
            src="/finease-removebg-preview-new.png"
            alt="Finease Logo"
            className="logo"
          />
        </div>
        <nav className="flex-1 p-4 space-y-3">
          {isLoggedIn && (
            <>
              <button
                className={`w-full text-left py-3 px-4 rounded-lg flex items-center ${activeView === "chat" ? "bg-blue-700" : "hover:bg-gray-700"}`}
                onClick={() => setActiveView("chat")}
              >
                <i className="fas fa-comment-dots text-xl mr-3"></i>{" "}
                <span className="text-lg">
                  {language === "en" ? "FAQ Chat" : "ಪ್ರಶ್ನೋತ್ತರ ಚಾಟ್"}
                </span>
              </button>
              <button
                className={`w-full text-left py-3 px-4 rounded-lg flex items-center ${activeView === "compare" ? "bg-blue-700" : "hover:bg-gray-700"}`}
                onClick={() => setActiveView("compare")}
              >
                <i className="fas fa-balance-scale-right text-xl mr-3"></i>{" "}
                <span className="text-lg">
                  {language === "en" ? "Analyze URLs" : "URL ವಿಶ್ಲೇಷಿಸಿ"}
                </span>
              </button>
              <button
                className={`w-full text-left py-3 px-4 rounded-lg flex items-center ${activeView === "history" ? "bg-blue-700" : "hover:bg-gray-700"}`}
                onClick={() => setActiveView("history")}
              >
                <i className="fas fa-history text-xl mr-3"></i>{" "}
                <span className="text-lg">
                  {language === "en" ? "History" : "ಇತಿಹಾಸ"}
                </span>
              </button>
              <button
                className={`w-full text-left py-3 px-4 rounded-lg flex items-center ${activeView === "account" ? "bg-blue-700" : "hover:bg-gray-700"}`}
                onClick={() => setActiveView("account")}
              >
                <i className="fas fa-user-circle text-xl mr-3"></i>{" "}
                <span className="text-lg">
                  {language === "en" ? "Account" : "ಖಾತೆ"}
                </span>
              </button>
              <button
                onClick={handleLanguageToggle}
                className="mt-4 w-full text-left py-2 px-4 rounded-lg flex items-center justify-center font-semibold bg-gray-600 hover:bg-gray-500"
              >
        ...
        <div className="p-4 border-t border-gray-700">
          <button
            className={`w-full flex items-center justify-center font-semibold text-lg py-3 px-4 rounded-lg ${isLoggedIn ? "bg-red-700 hover:bg-red-800" : "bg-green-700 hover:bg-green-800"}`}
            onClick={
              isLoggedIn
                ? handleSignOut
                : () => {
          ...
          >
            <i
              className={`fas ${isLoggedIn ? "fa-sign-out-alt" : "fa-sign-in-alt"} mr-3`}
            ></i>
            {isLoggedIn
              ? language === "en"
                ? "Sign Out"
